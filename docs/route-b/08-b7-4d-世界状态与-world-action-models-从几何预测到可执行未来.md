...
> [飞书原文](https://archebase.feishu.cn/docx/Ywecd7gKho7N9ax85g5ccwYfnVg) · 源修订 17
...
**前置知识：** 建议先完成 B1 状态空间与潜在动力学、B2 模型式规划、B4 视频世界模型和 B6 现代世界模型思想谱系。需要理解条件概率、扩散或 Flow Matching、相机几何、深度图和 Action Chunk。

**范围边界：** 本课中的 4D 指三维空间随时间变化，即 3D + time。它不是第四个空间维度，也不自动包含质量、摩擦、接触力、刚度、执行器动态和材料形变。4D 几何一致性是物理建模的重要条件，但不是完整物理充分条件。

# 从 B6 到 B7：四条现代路线在这里汇合

B6 已经建立思想坐标：决策世界模型提供规划与价值接口；JEPA 提供不必重建全部像素的预测表征；空间智能要求对象、几何与视角在三维世界中持续存在；生成式模拟器提供高容量的动作条件未来。B7 研究的 4D-WAM，正是这些能力开始被压进同一系统的位置。

| 上游路线 | 带入 4D-WAM 的能力 | 在本课中的审计问题 |
|-|-|-|
| 决策潜在模型 | 动作干预、rollout、奖励／价值与规划接口 | 联合模型的未来是否真正被用于选择动作 |
| JEPA 预测表征 | 抽象、低冗余、适合多步预测的 latent | 压缩是否保留接触点、间隙与控制关键细节 |
| 空间智能 | 持久对象、跨视角几何与时变三维状态 | 4D 一致性是否经得住遮挡、标定扰动与长时漂移 |
| 生成式模拟器 | 高容量未来生成、多模态条件与候选世界 | 视觉逼真是否对应动作因果与真实物理后果 |

因此，WAM 不是“视频生成模型再加一个动作头”，4D 也不是“视频再加深度”。真正的研究问题是：世界 token 与动作 token 是否通过合法因果路径共享信息；空间监督是否改善了控制相关表示；动作能否在世界生成完成之前低延迟输出；世界预测、动作生成与真实执行是否保持一致。
